...
pub fn exec_ws(ws: warp::ws::Ws, envs: HashMap<String, String>) -> impl warp::Reply {
    ws.on_upgrade(|websocket| async move {
        ...
        debug!(
            "spawning process with command: {:?} (tty? {})",
            command, tty
        );
        ...
        let (child_res, mut cmd_stdin, mut cmd_stdout) = if tty {
            ...
            let (r, w) = pty_master
                .split()
                .expect("could not split pty into read/write halves");
            ...
            let stdin = child
                ...
                .take()
                .map(Stdin::Normal)
                .unwrap_or(Stdin::DevNull);
            let stdout = child.stdout.take().map(Stdout::Normal).unwrap();
